...
# Sidebar Login - Fixed Version
st.sidebar.title("🛍️ CarryMe Store")
st.sidebar.markdown("**Premium Handcrafted Home Decor**")

login_expander = st.sidebar.expander("📱 Login / Register", expanded=not st.session_state.user)

with login_expander:
    mobile = st.text_input("Mobile Number *", value="9", max_chars=10, key="mobile_input")
    pin = st.text_input("4-digit PIN (Demo)", type="password", max_chars=4, key="pin_input")
    pincode = st.text_input("Delivery Pincode * (6 digits)", max_chars=6, key="pincode_input")
    ...
    if st.button("Login Now", type="primary"):
        if len(mobile) < 10:
            ...
            st.success(f"✅ Logged in successfully! Pincode: {pincode}")
            st.rerun()
...
# Rest of your pages (Home, Shop, Cart, etc.) remain the same as previous version
...
# Cart Page (with pre-filled pincode)
elif page == "🛒 Cart":
    # ... (keep your existing cart code with pre-filled pincode)
